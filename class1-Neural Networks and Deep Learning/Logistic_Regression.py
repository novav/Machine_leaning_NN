...
#导入数据
def load_dataset():
    train_dataset = h5py.File("train_cat.h5","r") #读取训练数据，共209张图片   // 64x64x3=12288
...
num_px = test_set_x_orig.shape[1] #图片的像素大小 = 64
print("num_px: {}".format(num_px))

train_set_x_flatten = train_set_x_orig.reshape(train_set_x_orig.shape[0],-1).T #原始训练集的设为（12288*209）
print("test_set_x_orig: {}".format(train_set_x_flatten.shape[0]))
test_set_x_flatten = test_set_x_orig.reshape(test_set_x_orig.shape[0],-1).T #原始测试集设为（12288*50）
print("test_set_x_flatten: {}".format(test_set_x_flatten.shape[0]))
