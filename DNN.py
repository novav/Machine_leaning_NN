...
def initialize_parameters_deep(layer_dims):
    ...
    L = len(layer_dims)
    print("initialize_parameters_deep L = {}".format(L))
    for l in range(1, L):
        # 初始化为随机值
        # print("L a= {}".format(l))
        # print("L a=l {}".format(layer_dims[l]))
        # print("L b=l-1 {}".format(layer_dims[l - 1]))
        # print("L c=VB {}".format(np.sqrt( [l - 1])))
        # print("L d=b/c {}".format( (layer_dims[l - 1]) / np.sqrt(layer_dims[l - 1])) )
        parameters['W' + str(l)] = np.random.randn(layer_dims[l], layer_dims[l - 1]) / np.sqrt(layer_dims[l - 1])
        parameters['b' + str(l)] = np.zeros((layer_dims[l], 1))  # 初始化为0
    # print("L = {}".format(parameters))
    # print(" = {}".format(np.random.randn(1, 2.2)))
    return parameters
...
def L_model_forward(X, parameters):
    ...
    # print("L_model_forward L = {}".format(len(parameters))) = 8
    for l in range(1, L):
        A_prev = A
...
my_predicted_image = predict(my_image, my_label_y, parameters)  ## TEST

plt.imshow(image)
print ("y = " + str(np.squeeze(my_predicted_image)) + ", 你的L层神经网络模型检测到它是一张 \""
