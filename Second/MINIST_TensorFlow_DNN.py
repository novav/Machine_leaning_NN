...
MNIST_data_folder = "O:\\xinxin.shi\\AI\\demos\\cat\\Second\\MNIST_data\\"
...
# 第1层，卷积层
# 输入1个28x28图像 -> 输出32个28x28特征图像
# 初始化W为[5,5,1,32]的张量，表示卷积核大小为5*5，第一层网络的输入和输出神经元个数分别为1和32
# 首先在每个5x5网格中，提取出32张特征图。其中weight_variable中前两维是指网格的大小，
# 第三维的1是指输入通道数目，第四维的32是指输出通道数目（也可以理解为使用的卷积核个数、得到的特征图张数）。
...
# tf.nn.relu 修正作用，修正的线性：max(features, 0)，比较特征值和0 的大小，小于0的用0来取代
...
# 输入32个14x14矩阵，与5x5矩阵卷积，输出为64个14x14矩阵
...
for i in range(200):
    batch = mnist.train.next_batch(20)  # 50 -》20
    if i % 10 == 0:
        # 每100次输出一次日志
...
for i in range(100):
    batch = mnist.test.next_batch(20)  # 50 -》20
    if i % 10 == 0:
